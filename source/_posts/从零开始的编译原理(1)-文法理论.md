...
> 说起来 “句柄” 这个翻译的滥用实在是中文计算机领域的一大丑闻。将 Handle 翻译成句柄最早就出现在编译领域中。此处 Handle 确实是句型中的成分，而 Handle 也确实有把手的意思，所以 “句柄” 这个翻译并非没有道理。但是如果 Handle 指代的是用于操纵某些资源的标识，如文件描述符，那么所谓 “句柄” 就完全没有道理了，翻译成 “把手”、“柄” 乃至 “抓手”、“编号” 都比 “句柄” 好。“句柄” 完全不知所谓。
...
## 六、乔姆斯基范式
在最后，我们终于可以为文法理论部分做一个总结。在这篇文章中所写的全部内容，都来源于乔姆斯基（Avram Noam Chomsky）20世纪50年代提出的生成语法理论。他的理论源自语言学的研究，却实现了语言学从人文科学到自然科学的转向。恰恰也是在这个时间，编译理论开始发展。两门看似不相关的学科走到了一起。

作为一个补充，同时也为了衔接后面的内容，在最后提出乔姆斯基范式的内容。

和前面所定义的一样，乔姆斯基将所有文法都定义为四元组 $G = (V_n. V_t, P, Z)$。

而根据对规则 $P$ 所施加的限制的不同，可以将语言分为四型：

- **0 型文法**：$P: u ::=v$，其中 $u \in V^+, v \in V^*, V = V_n \cup V_t$。也称为短语结构文法。规则的左部和右部都可以是符号串，一个短语可以产生另一个短语。
- **1 型文法**：$P: xUy ::= xuy$ 其中 $U \in V_n, x,y,u \in V^*$。也称为上下文敏感或上下文有关。这意味着只有在 $x$ 和 $y$ 这样的上下文中才能把 $U$ 改写为 $u$。
- **2 型文法**：$P: U::=u$，其中 $U \in V_n, u \in V^*$。也称为上下文无关文法。意味着在任何情况下都可以将 $U$ 改写为 $u$，而不需要考虑上下文。
- **3 型文法**：$P: U::=t \text{or} U::=Wt$ 其中 $U,W \in V_n, t \in V_t$（左线性）或 $P: U::=t \text{or} U::=tW$ 其中 $U,W \in V_n, t \in V_t$（右线性）。也称正则文法。意味着终结符只能在规则的一侧生成。

我们之前对文法的定义属于 2 型文法和 3 型文法。这也是我们在未来真正需要用到的两型文法。其中 3 型文法，或正则文法，主要用于划分单词，在编译过程的词法分析部分使用；而 2 型文法，或上下文无关文法，则用于将顺序的单词序列解析为文法树，在编译过程的语法分析部分使用。
