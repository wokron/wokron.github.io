...
<!-- 由于**所有的句柄都是规则右侧的符号串**，所以我们可以得到确定优先关系的三条规则：
1. 在同一条规则右侧出现的终结符优先级相等。例如有 $A \rightarrow aBcDe$，则有 $a$ ≐ $c$ 和 $c$ ≐ $e$。
2. 非终结符最左侧可能推出的终结符的优先级大于该非终结符位于规则中时相邻的左侧终结符。例如有 $A \rightarrow aBc$ 和 $$
3. 非终结符最右侧可能推出的终结符的优先级大于该非终结符位于规则中时相邻的右侧终结符。 -->

<!-- 然而，对于给定的文法，我们不能在一开始就确定各个终结符间的优先关系。不过 -->

<!-- 通过上面的算法我们可以得知，在算符优先分析法中，归约的依据是 **“句柄” 中的终结符的优先级大于 “句柄” 外的终结符**。在这种情况下，句柄左侧的终结符均小于句柄，因此会不断移进，直到句柄的所有符号都移入栈中，此时句柄右侧的终结符大于句柄，因此将句柄归约。 -->


<!-- 从该算法中我们可以得知，在算符优先分析法中，“句柄” 的识别取决于其最右侧的终结符和句柄

算符优先分析法只适用于**算符文法**（Operator Grammar, OG）。如果文法 $G$ 中没有形如 $U \rightarrow ...VW...$，其中 $V,W \in V_n$ 的规则（即存在两个非终结符相邻的规则），则称 $G$ 为算符文法。

在算符优先分析法中，何时进行归约取决于

如果一个 2 型文法是算符文法，则我们能按照如下规则确定 -->
