...
解决这个问题的关键在于使信号处理函数返回时不再执行同一条语句，这需要 `longjump` 的支持。准确来说是 `siglongjump`。
...
SIGALRM、SIGVTALRM 和 SIGPROF 信号的产生源自相同的系统调用 `setitimer`。该系统调用设置一个异步的计时器，当计时器到期时会触发对应的信号。这些信号的默认处置会终止当前进程。
