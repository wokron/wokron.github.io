...
很明显，该 DFA 可接受 0 个或多个 $a$ 组成的符号串。但是该 DFA 并不是接受该语言的最简 DFA，因为下面的 DFA 同样能够接受：
...
- **等价状态**指 DFA 中**表现相同**的不同状态。即对于任何**相同输入**，都转移到**相同的状态**或**其他等价状态**的不同状态。因为表现相同，所以等价的不同状态可以**合并为同一个状态**，而不会影响 DFA 所表达的语言。
...
由此我们可以得出将 DFA 最小化的方法。首先，我们要求对于 DFA 状态机图中的任意状态，从开始状态**是否可达**。如果不可达则为多余状态，将其删去即可。这只需要计算状态机图的传递闭包。

之后，我们需要将 DFA 中的所有状态划分为不同的**等价状态集**，同一集合中的状态相互等价。并进行状态合并，将等价状态集视为状态，得到最小化的 DFA。
...
下面介绍的一种判定等价状态集的方法可以称为**分割法**。思路是假定不同状态属于同一个等价状态集，再根据集合中状态转移后结果的不同重新划分集合。不断重复，得到最终结果。具体方法描述如下：
1. 令初始等价状态集的集合 $E^{(0)} = \{S_0^{(0)}, S_1^{(0)}\}$，其中 $S_0^{(0)} = \{s | s\text{不是接受状态}\}$，$S_1^{(0)} = \{s | s\text{是接受状态}\}$
2. 第 $i$ 步迭代，对任意等价状态集 $S_j^{(i-1)} \in E^{(i-1)}$，对其中任意状态 $s_k \in S_j$，定义等价状态集序列 $seq_{jk} = S_{c_1}, S_{c_2}, ..., S_{c_n}$，其中 $S_{c_m} = \sigma(s_k, c_m), c_n \in \Sigma$，$\Sigma$ 为符号集。
3. 若存在 $k_1, k_2, ...k_p$，其中 $seq_{jk_1}, seq_{jk_2}, ...seq_{jk_p}$ 均相同，则定义第 $i$ 步的一个等价集为 $S^{(i)}_q = \{s_{k_1}, s_{k_2}, ..., s_{k_p}\}$。据此将集合 $S^{(i-1)}_j$ 划分为新的等价集，并最终得到 $E^{(i)}$
4. 重复迭代 2、3 步，直到等价集不再发生变化。此时得到的 $E^{(q)}$ 即为最终的等价状态集集合。
